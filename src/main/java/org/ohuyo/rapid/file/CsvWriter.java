...
public class CsvWriter {
	...
	public CsvWriter(Writer paramWriter, char paramChar) {
		...
		this.outputStream = new PrintWriter(paramWriter);
		this.userSettings.Delimiter = paramChar;
		this.initialized = true;
	}

	public CsvWriter(OutputStream paramOutputStream, char paramChar,
	...
	public void write(String paramString, boolean paramBoolean)
		...
		if ((!bool)
				...
				&& ((paramString.indexOf(this.userSettings.TextQualifier) > -1)
						|| (paramString.indexOf(this.userSettings.Delimiter) > -1)
						|| ((!this.useCustomRecordDelimiter) && ((paramString
								.indexOf('\n') > -1) || (paramString
								.indexOf('\r') > -1)))
						|| ((this.useCustomRecordDelimiter) && (paramString
								.indexOf(this.userSettings.RecordDelimiter) > -1))
						|| ((this.firstColumn) && (paramString.length() > 0) && (paramString
								.charAt(0) == this.userSettings.Comment)) || ((this.firstColumn) && (paramString
						.length() == 0))))
			bool = true;
		if ((this.userSettings.UseTextQualifier) && (!bool)
				&& (paramString.length() > 0) && (paramBoolean)) {
		...
		if (bool) {
			...
			if ((this.firstColumn) && (paramString.length() > 0)
					&& (paramString.charAt(0) == this.userSettings.Comment))
		...
		if (!this.initialized) {
			if (this.fileName != null)
				this.outputStream = new PrintWriter(new OutputStreamWriter(
						new FileOutputStream(this.fileName), this.charset));
			this.initialized = true;
		}
	}
