...
<div class="pageContent">
	<form method="post" class="pageForm required-validate"
		action="${ctx}/cust/paynotice/pay4SupplementDeposit.do"
		...
		<div class="pageFormContent" layoutH="65">
			...
			<p>
				<label>协议编号：</label>
				<input type="text" readonly="readonly" value='${tagmt.agmtNo}' />
			</p>
			<p>
				<label>应补交保证金：</label>
				<input type="text" readonly="readonly" value='<fmt:formatNumber value="${-tagmt.redundantDeposit}" pattern="#,##0.00#" />' style="text-align: right;color: red;"/><span class="info">元</span>
			</p>
			<p>
				<label>已支付金额：</label>
			...
			<p>
				<label>剩余应支付：</label>
				<input type="text" readonly="readonly" value='<fmt:formatNumber value="${(-tagmt.redundantDeposit) - tpayNotify.havepayAmt}" pattern="#,##0.00#" />' style="text-align: right;color: red;"/><span class="info">元</span>
				<input type="hidden" id="shouldPayAmt" value="${(-tagmt.redundantDeposit) - tpayNotify.havepayAmt}" />
			</p>
			<p>
				<label>银行账号：</label>
