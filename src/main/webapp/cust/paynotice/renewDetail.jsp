...
<div class="pageContent">
	<div class="pageFormContent" layoutH="68">
		<p>
			<label>续保编号：</label> <input type="text" value="${trenew.irenew}"
				readonly="readonly">
		</p>
		<p>
			<label>申请时间：</label> <input type="text"
				value="<fmt:formatDate value="${trenew.renewTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
				readonly="readonly">
		</p>
		<p>
			<label>申请期限：</label>
		...
		<p>
			<label>续保状态：</label> <input type="text"
				value="<posmall:dict dictCode="${trenew.renewStatus}" dictType="renew_status" />"
				readonly="readonly">
		</p>
		<p>
			<label>支付状态：</label> <input type="text"
				value="<posmall:dict dictCode="${trenew.payStatus}" dictType="pay_status" />"
				readonly="readonly">
		</p>
		<p>
						...
						<c:forEach items="${detailList}" var="list">
							<tr>
								...
								<td align="center">&nbsp;<fmt:formatDate pattern="yyyy-MM-dd"
										value="${list.lifeStartTime}" /></td>
								<td align="center">&nbsp;<fmt:formatDate pattern="yyyy-MM-dd"
										value="${list.lifeEndTime}" /></td>
								<td></td>
							</tr>
						</c:forEach>
					</tbody>
