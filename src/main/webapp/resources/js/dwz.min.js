...
function initUI(_box){
	...
	if ($.fn.datepicker){
		$('input.date', $p).each(function(){
			...
			if($this.attr("afterStep")) opts.afterStep = $this.attr("afterStep");
...
var alertMsg = {
	...
	_open: function(type, msg, buttons){
		...
		if (this._types.info == type || this._types.correct == type){
			this._closeTimer = setTimeout(function(){alertMsg.close();}, 6000);
		} else {
			$(this._bgId).show();
		}
		...
		return this.each(function(){
			...
			function generateCalendar(dp){
				...
				$days.not('.disabled').click(function(){
					...
					if (!dp.hasTime()) {
						...
						closeCalendar(); 
						//添加 cheny
						if(dp.opts.afterStep == 2){
							dateAfterOper(dp.formatDate(dp.changeDay($day.attr("day"), $day.attr("chMonth"))));
						}
					} else {
			...
			$this.click(function(event){
				...
				$(setting.okBut$).click(function(){
					...
					//添加 cheny
					if(dp.opts.afterStep == 2){
						dateAfterOper(dp.formatDate(date));
					}
				});
